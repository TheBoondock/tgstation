...
/datum/dna/proc/generate_unique_features()
	...
	if(features["plasma_hair"])
		L[DNA_PLASMA_HAIR_BLOCK] = construct_block(SSaccessories.plasma_hair_list.Find(features["plasma_hair"]), length(SSaccessories.plasma_hair_list))
...
/datum/dna/proc/update_uf_block(blocknumber)
	...
	switch(blocknumber)
		...
		if(DNA_PLASMA_HAIR_BLOCK)
			set_uni_feature_block(blocknumber, construct_block(SSaccessories.plasma_hair_list.Find(features["plasma_hair"]), length(SSaccessories.plasma_hair_list)))
...
/mob/living/carbon/human/updateappearance(icon_update = TRUE, mutcolor_update = FALSE, mutations_overlay_update = FALSE)
	...
	if(dna.features["plasma_hair"])
		dna.features["plasma_hair"] = SSaccessories.plasma_hair_list[deconstruct_block(get_uni_feature_block(features, DNA_PLASMA_HAIR_BLOCK), length(SSaccessories.plasma_hair_list))]
